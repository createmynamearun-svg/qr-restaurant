    ...
    <section className="py-16 md:py-24 bg-muted/30">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Left: Order status pipeline card */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-white/60 dark:bg-card/60 backdrop-blur-sm border border-white/40 dark:border-border/40 rounded-2xl shadow-lg p-6"
          >
            ...
            <div className="space-y-4">
              {orders.map((order, i) => (
                <motion.div
                  ...
                  className="flex items-center justify-between p-3 rounded-xl bg-background/60 border border-border/30"
                >
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-gradient-to-br from-orange-200/60 via-amber-100/40 to-yellow-200/60 dark:from-orange-900/20 dark:via-amber-900/10 dark:to-yellow-900/20 backdrop-blur-sm border border-white/40 dark:border-border/40 rounded-2xl shadow-lg p-6 flex items-center justify-center min-h-[280px]"
          >
            <div className="text-center">
              <motion.div
                animate={{ y: [0, -8, 0] }}
                transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
                className="w-24 h-24 rounded-2xl bg-white/80 dark:bg-card/80 shadow-lg flex items-center justify-center mx-auto mb-4"
              >
