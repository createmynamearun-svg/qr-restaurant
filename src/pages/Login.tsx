      ...
      try {
        ...
        if (data.session?.user) {
          ...
          if (roleData?.role) {
            ...
            navigate(routes[roleData.role] || '/roles');
            return;
          }
        }
      } catch {
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-10 w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="relative">
              ...
              <Input type="email" placeholder="you@restaurant.com" value={email} onChange={e => setEmail(e.target.value)} disabled={loading} autoComplete="email" className="pl-10 bg-gray-100 border-0 h-12 rounded-xl text-gray-800 placeholder:text-gray-400 focus-visible:ring-blue-500" />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
              <Input type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} disabled={loading} autoComplete="current-password" className="pl-10 pr-10 bg-gray-100 border-0 h-12 rounded-xl text-gray-800 placeholder:text-gray-400 focus-visible:ring-blue-500" />
